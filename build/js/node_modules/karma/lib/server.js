...
class Server extends KarmaEventEmitter {
  ...
  bufferIncoming (socket, buffered) {
    socket.pause()

    const cleanup = () => buffered.delete(socket)
  ...
  async start () {
    ...
    try {
      ...
      this._listener = (socket) => this.bufferIncoming(socket, this._buffered)

      this._boundServer.on('connection', (socket) => this._listener(socket))
  ...
  async _start (config, launcher, preprocess, fileList, capturedBrowsers, executor, done) {
    ...
    const afterPreprocess = () => {
      ...
      webServer.listen(this._boundServer, () => {
        ...
        this._boundServer.removeListener('connection', this._listener)

        for (const socket of this._buffered) {
